...
from rbp_app.services import payment_events, stripe_gateway
from rbp_app.services import stripe_entitlements as entitlement_sync
...
class TestStripeCheckoutFoundation(StripeFoundationTestCase):
    ...
    def test_verify_webhook_requires_signature_and_uses_webhook_secret(self):
        ...
        with patch.object(stripe_gateway, "get_stripe_module", return_value=stripe):
            with patch.object(stripe_gateway, "get_stripe_webhook_secret", return_value="webhook_secret_test_value"):
                event = stripe_gateway.verify_webhook_event(b"{}", "t=1,v1=sig")

        self.assertEqual(event["id"], "evt_123")
        construct_event.assert_called_once_with(b"{}", "t=1,v1=sig", "webhook_secret_test_value")
...
class TestSubscriptionEntitlementSync(StripeFoundationTestCase):
    def test_subscription_entitlements_are_created_and_revoked_from_plan_apps(self):
        ...
        with (
            patch.object(entitlement_sync, "doctype_exists", return_value=True),
            patch.object(entitlement_sync, "record_audit_event"),
            patch.object(
                entitlement_sync.frappe,
                "db",
                SimpleNamespace(
                    exists=MagicMock(return_value=True),
            ...
            patch.object(entitlement_sync.frappe, "get_doc", side_effect=get_doc),
        ):
            result = entitlement_sync.sync_entitlements_for_subscription(subscription, active=True)
