...
class TestMilestone8Entitlements(TestCase):
	...
	def test_list_my_entitlements_forwards_include_inactive(self):
		with patch.object(entitlements, "get_user_entitlements", return_value=[]) as get_user_entitlements:
			result = entitlements.list_my_entitlements(user="member@example.com", include_inactive=True)

		self.assertEqual(result["entitlements"], [])
		get_user_entitlements.assert_called_once_with(user="member@example.com", include_inactive=True)
